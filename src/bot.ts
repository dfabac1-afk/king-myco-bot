...
export class KingMycoBot {
  ...
  private setupMenuButton(): void {
    const commands = [
      ...
      { command: 'dailywinners', description: '📅 View daily winners history' },
      { command: 'sporeleaderboard', description: '⭐ View spore leaderboard' }
    ];
  ...
  private setupHandlers(): void {
    ...
    this.bot.onText(/\/dailywinners(@\w+)?/i, async (msg) => { try { await this.handleDailyWinners(msg); } catch (e) { console.error('[DAILYWINNERS] Error:', e); this.bot.sendMessage(msg.chat.id, 'Error: Could not process /dailywinners'); } });
    this.bot.onText(/\/sporeleaderboard(@\w+)?/i, async (msg) => { try { await this.handleSporeLeaderboard(msg); } catch (e) { console.error('[SPORELEADERBOARD] Error:', e); this.bot.sendMessage(msg.chat.id, 'Error: Could not process /sporeleaderboard'); } });
  ...
  private async handleCallbackQuery(query: TelegramBot.CallbackQuery): Promise<void> {
    ...
    switch (data) {
      ...
      case 'edu_scams':
        ...
        break;
      case 'daily_winners':
        this.handleDailyWinners({ chat: { id: chatId } } as TelegramBot.Message);
        break;
      case 'back_main':
  ...
  private async handleMessage(msg: TelegramBot.Message): Promise<void> {
    ...
    // Only respond if message contains MYCO, $MYCO, king myco (case insensitive) or ends with ?
    const lowerText = text.toLowerCase();
    const containsMyco = lowerText.includes('myco') || lowerText.includes('$myco') || lowerText.includes('king myco');
    const isQuestion = text.trim().endsWith('?');
    
    if (!containsMyco && !isQuestion) {
      return; // Ignore messages that don't match criteria
    }
  ...
  private async handleButtonClick(chatId: number, userId: number, userName: string): Promise<void> {
    try {
      ...
      if (result.success) {
        ...
        // PUBLIC ANNOUNCEMENT - Celebrate the push and encourage competition!
        const announcements = [
          `🎉 ${userName} just pushed the button! They now have ${userRank?.clicks || 1} total pushes!\n\n💪 Can YOU beat their record? Push the button and compete! 🏆`,
          `⚡ BOOM! ${userName} pushed the button! Current streak: ${userRank?.clicks || 1} pushes!\n\n🔥 Don't let them take the lead! Your turn to push! 🔘`,
          `🍄 ${userName} has activated the fungal power! Total pushes: ${userRank?.clicks || 1}!\n\n👑 Will you challenge King Myco's chosen one? Push now! 💎`,
          `🧙 The mycelium awakens! ${userName} pushed the button ${userRank?.clicks || 1} time${userRank?.clicks !== 1 ? 's' : ''}!\n\n✨ The competition intensifies... Who will be next? 🎯`,
        ];
        
        const randomAnnouncement = announcements[Math.floor(Math.random() * announcements.length)];
        
        // Public announcement with current rank
        const rankInfo = userRank ? `\n\n🏅 Current Rank: #${userRank.rank}` : '';
        await this.bot.sendMessage(chatId, `${randomAnnouncement}${rankInfo}`, {
          reply_markup: {
            inline_keyboard: [[
        ...
        // Send cooldown message privately
    ...
    const options = {
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: '📅 Daily Winners', callback_data: 'daily_winners' }],
          [{ text: '⬅️ Back', callback_data: 'back_main' }],
        ],
      },
    } as TelegramBot.SendMessageOptions;
  ...
  private async handleDailyWinners(msg: TelegramBot.Message): Promise<void> {
    const chatId = msg.chat.id;
    
    // Get daily wins champions (most days won)
    const champions = await this.buttonContest.getDailyWinsLeaderboard(10);
    const championsText = this.buttonContest.formatDailyWinsLeaderboard(champions);
    
    // Get recent daily winners history
    const history = await this.buttonContest.getDailyWinnersHistory(15);
    const historyText = this.buttonContest.formatDailyWinnersHistory(history);
    
    const fullMessage = `${championsText}\n\n${historyText}`;
    
    const options = {
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔘 Push Button', callback_data: 'button_push' }],
          [{ text: '🏆 Overall Leaderboard', callback_data: 'menu_leaderboard' }],
          [{ text: '⬅️ Back', callback_data: 'back_main' }],
        ],
      },
    } as TelegramBot.SendMessageOptions;
    
    this.bot.sendMessage(chatId, fullMessage, options);
  }
  ...
  private async handleSporeLeaderboard(msg: TelegramBot.Message): Promise<void> {
    ...
    try {
      if (!this.supabase) {
        this.bot.sendMessage(chatId, '⚠️ **Spore System Not Available**\n\nThe spore system requires Supabase integration.\n\n**Setup Steps:**\n1. Configure SUPABASE_URL and SUPABASE_KEY in your .env\n2. Run `supabase-setup.sql` in your Supabase SQL Editor\n3. Restart the bot\n\nFor help, check SUPABASE_DAILY_WINNERS_SETUP.md');
        return;
      }
...
📊 No users have earned spores yet!

**How to Earn Spores:**
🔘 Push the button (/buttonpush) - 10 spores per push
🎯 Complete quests (coming soon)
🏆 Win daily button contests

**Start earning now!** Use /buttonpush to begin.
        `;
        ...
        const options = {
          reply_markup: {
            inline_keyboard: [
              [{ text: '🔘 Push Button', callback_data: 'button_push' }],
              [{ text: '🏆 Button Leaderboard', callback_data: 'menu_leaderboard' }],
              [{ text: '⬅️ Back', callback_data: 'back_main' }],
            ],
          },
        } as TelegramBot.SendMessageOptions;
        
        this.bot.sendMessage(chatId, noDataMessage, options);
        return;
      }

      let leaderboardText = '⭐ **SPORE LEADERBOARD** ⭐\n\n';
      leaderboardText += '🏆 Top Spore Earners:\n\n';
      ...
      leaderboard.forEach((entry: any, index: number) => {
        ...
        let displayName = '';
        if (entry.telegramName) {
          displayName = entry.telegramName;
        } else if (entry.walletAddress) {
          displayName = `${entry.walletAddress.substring(0, 6)}...${entry.walletAddress.substring(entry.walletAddress.length - 4)}`;
        } else {
          displayName = 'Unknown User';
        }
        
        leaderboardText += `${medal} **${displayName}**\n`;
        leaderboardText += `   💰 ${entry.totalSpores.toLocaleString()} spores`;
        
        if (entry.questsCompleted > 0) {
          leaderboardText += ` | 🎯 ${entry.questsCompleted} quests`;
        }
        
        if (entry.buttonPushes > 0) {
          leaderboardText += ` | 🔘 ${entry.buttonPushes} pushes`;
        }
      ...
      leaderboardText += '---\n💡 **Earn spores by:**\n';
      leaderboardText += '🔘 Pushing the button every 30 minutes\n';
      leaderboardText += '🏆 Winning daily button contests\n';
      leaderboardText += '🎯 Completing quests at https://kingmyco.com/quests';

      const options = {
        reply_markup: {
          inline_keyboard: [
            [{ text: '🔘 Push Button', callback_data: 'button_push' }],
            [{ text: '🎯 Complete Quests', url: 'https://kingmyco.com/quests' }],
            [{ text: '🏆 Button Leaderboard', callback_data: 'menu_leaderboard' }],
            [{ text: '📅 Daily Winners', callback_data: 'daily_winners' }],
            [{ text: '⬅️ Back', callback_data: 'back_main' }],
          ],
      ...
      console.error('[SPORE_LEADERBOARD] Error:', error);
      this.bot.sendMessage(chatId, '❌ **Error Loading Spore Leaderboard**\n\nPossible issues:\n• Database tables not initialized\n• Supabase credentials incorrect\n• Connection timeout\n\nRun `supabase-setup.sql` in Supabase and verify your .env settings.');
    }
  }
...
*Complete quests on kingmyco.com to earn more spores!*
  ...
  public start(): void {
    ...
    // Start daily winner announcement timer (every 24 hours)
    this.startDailyWinnerAnnouncements();
  }
  
  private startDailyWinnerAnnouncements(): void {
    ...
    console.log(`[DAILY_WINNER] First announcement in ${Math.round(msUntilMidnight / 1000 / 60)} minutes`);
    
    // Schedule first announcement at midnight
    setTimeout(() => {
      this.announceDailyWinner();
  ...
  private async announceDailyWinner(): Promise<void> {
    try {
      console.log('[DAILY_WINNER] Announcing daily winner...');
      const winner = this.buttonContest.getDailyWinner();
      
      if (!winner) {
        console.log('[DAILY_WINNER] No button pushes in the last 24 hours');
        return;
      }
      
      // Save winner to history BEFORE resetting stats
      this.buttonContest.saveDailyWinner(winner);
      
      // Get their total daily wins (await async call)
      const dailyWinsLeaderboard = await this.buttonContest.getDailyWinsLeaderboard();
      const championStats = dailyWinsLeaderboard.find(c => c.userId === winner.userId);
      const totalDailyWins = championStats?.wins || 1;
...
🔘 Daily Pushes: ${winner.dailyPushes}
💎 Total Pushes: ${winner.clicks}
🏅 Current Rank: #${winner.rank}
� Total Daily Wins: ${totalDailyWins}
...
🔘 Push the button every 30 minutes to compete!
      ...
      // Get all active chats (you may need to track these)
      // For now, we'll just log it. In production, send to your announcement channel/group
      console.log('[DAILY_WINNER]', announcement);
      
      // If you have a specific announcement chat ID, uncomment and set it:
