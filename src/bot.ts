...
✨ Avoid predictions & absolutes
✨ Grow wiser with each conversation

💡 Examples:
/askkingmyco What's your take on DeFi?
/askkingmyco How should I think about risk?
/askkingmyco What's the future of blockchain?

Each conversation is unique - ask again for deeper wisdom!
    `;
  ...
  private async handleKingMycoMenu(msg: TelegramBot.Message): Promise<void> {
      console.log('handleKingMycoMenu triggered');
    const chatId = msg.chat.id;

    const kingMycoMessage = `
👑 **KING MYCO - The Wisdom Coin**

Welcome to the legendary realm of King Myco!

🪙 **Token Information:**
• Name: King Myco
• Contract: \`9BySdih23rwDPZB8auQXX9k5u6a2Nk4GSDji2MB6pump\`
• Blockchain: Solana
• Type: Meme Coin with Purpose

📖 **About King Myco:**
King Myco isn't just a coin - it's a philosophy. Blending wisdom with whimsy, we believe in the power of community, learning, and enlightened investing in the crypto space.
...
📊 **Quick Stats:**
To get live stats, use:
/ca 9BySdih23rwDPZB8auQXX9k5u6a2Nk4GSDji2MB6pump

🧙 **Our Mission:**
To bring wisdom, wonder, and whimsy to the crypto community through education, analysis, and thoughtful guidance.
...
Choose what you'd like to explore:
    ...
    const options = {
      reply_markup: {
        inline_keyboard: [
          [
            { text: '💰 Token Stats', callback_data: 'myco_stats' },
            { text: '📚 Learn More', callback_data: 'myco_learn' },
          ],
          [
            { text: '🧙 Ask King Myco', callback_data: 'myco_wisdom' },
            { text: '📊 Market Data', callback_data: 'myco_market' },
          ],
          [
  ...
  private async handleCALookup(
    msg: TelegramBot.Message,
    match: RegExpExecArray | null
  ): Promise<void> {
      console.log('handleCALookup triggered', match);
    const chatId = msg.chat.id;
    const contractAddress = match?.[1];

    if (!contractAddress) {
      this.bot.sendMessage(chatId, '❌ Please provide a contract address.\nUsage: /ca <contract_address>');
      return;
    }
    ...
    try {
      ...
      if (!tokenInfo) {
        this.bot.editMessageText(
          '❌ Token not found. Please check the contract address and try again.',
          {
            chat_id: chatId,
...
24h Price Change: ${tokenInfo.priceChange24h > 0 ? '📈' : '📉'} ${tokenInfo.priceChange24h.toFixed(2)}%
24h Buys: ${tradingActivity?.buys24h || 'N/A'}
24h Sells: ${tradingActivity?.sells24h || 'N/A'}
      `;

      // Get AI insights
      const aiInsight = await this.openai.askAboutProject(
        tokenInfo.name,
        context
      );
...
Buys: ${tradingActivity?.buys24h || 'N/A'}
Sells: ${tradingActivity?.sells24h || 'N/A'}
